                ...
                <div class="meetup-lookup centered">
                    <div class="level">
                        <div class="level-left">
                            ...
                            <div v-if="getMeetups.data && getMeetups.data.length > 0"
                                class="level-item"
                            >
                                <span
                                    >Meetups in
                                    {{ getMeetups.data[0].location }}</span
                                >
                            </div>
                            <div
                                v-if="category && routeCategory"
                                class="level-item"
                            >
                                <button
        ...
        <div class="container">
            <section class="section page-find">
                <div
                    ...
                    v-if="getMeetups.data.length > 0"
                >
                    <div
                        v-for="meetup of getMeetups.data"
                        :key="meetup.id"
                        ...
                        >
                            <div class="meetup-card-find-content">
                                ...
                                <div class="meetup-card-find-content-info">
                                    <p
                                        class="title is-4 no-padding is-marginless m-b-xs"
                                    >
                                        {{ meetup.title }}
                                    </p>
                ...
                <div v-else>
                    <span class="tag is-warning is-large"
                        >No meetups found :( You might try to change search
                        criteria (:</span
                    >
                </div>
            </section>
...
export default {
  ...
  created() {
    ...
    /* if (this.category) {
            axios
                .get(`/app/meetupCategory?q=${this.category}`)
                .then(res => {
                    console.log(res.data);
                    this.$store.dispatch("setMeetups", res.data);
                })
                .catch(err => {
                    console.log(err);
                });
  ...
  methods: {
    fetchMeetups() {
      if (this.query) {
        this.filter["location"] = this.query
          .toLowerCase()
          .replace(/[\s,]+/g, "")
          .trim();
      }
    ...
    initMeetups() {
      if (this.$route.params.category) {
        this.$router.push({ name: "meetupFind" });
        this.query = "";
      }
      this.$store.dispatch("getList", 0);
      this.query = "";
